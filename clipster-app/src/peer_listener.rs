...
//! peers on the LAN can pull from us.
...
pub async fn run(core: Arc<ClipsterCore>, port: u16) -> anyhow::Result<()> {
    ...
    tracing::info!(%bind, device = %core.identity.device_id, "Tauri peer listener up");
    ...
    loop {
        ...
        tokio::spawn(async move {
            match acceptor.accept(stream).await {
                ...
                Err(e) => {
                    tracing::debug!(error = %e, "Tauri peer TLS handshake failed");
                }
            }
        });
    }
